...
from universal_mcp_apollo.app import ApolloApp

@pytest.fixture
def app_instance():
    """Provides a ApolloApp instance for tests."""
    mock_integration = MagicMock()
    mock_integration.get_credentials.return_value = {"access_token": "dummy_access_token"}

    return ApolloApp(integration=mock_integration)

def test_universal_mcp_apollo_app_initialization(app_instance):
    """
    Test that the ApolloApp instance is initialized correctly with a name.
    """
    ...
    assert app_instance.name == "apollo", "ApolloApp instance has unexpected name."


def test_universal_mcp_apollo_tool_docstrings_format(app_instance):
    """
    Test that each tool method in ApolloApp has a well-formatted docstring,
...
def test_universal_mcp_apollo_tools_are_callable(app_instance):
    """
    Test that each tool method returned by list_tools in ApolloApp is callable.
    """
    tools = app_instance.list_tools()
    assert isinstance(tools, list), "list_tools() should return a list."
